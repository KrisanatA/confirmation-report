...
#| fig-cap: "An ambulance transfer network in Victoria between residential aged care facilities and hospitals."
...
#| fig-cap: "An interactive ambulance transfer network in Victoria between residential aged care facilities and hospitals."
#| code-summary: "Visualisation"
...
Current tools, such as `tidygraph`, have several limitations that are important for exploratory data analysis. First, temporal data are not easy to clean within `tidygraph`, and there is no direct support for filtering temporal networks. As a result, issues such as filling missing timestamps and inconsistent time periods are currently handled outside the network context, which may affect the network's validity. Second, support for the data sampling method is limited, with most methods working outside of the `tidygraph` object. Finally, visualisation capabilities are constrained by the `ggraph` package, as layout calculations are handled internally, making it difficult to access and reuse layout information for expanding the visualisation method. 

To address these limitations, I will be creating a new data object. For temporal information, this object will integrate temporal and network topology jointly. Rather than converting an `igraph` to a `tibble` object, it will be built on top of a `tibble` object, allowing full compatibility with other tools. Additionally, the object will be designed to work with the `ggplot2` package for better expansion and application of other methods for network visualisation.


## Part B: An Exploratory Analysis of the COVID-19 Impact on Ambulance Efficiency and Transfer Volume

The efficiency of ambulance transfer is critical in both scheduled and emergency cases to minimise the resources used and the effect on patient outcomes. In scheduled transfers, the longer the transfer, the fewer ambulances are available in the system, which potentially limits the number of ambulances available during the peak demand or emergency state. In emergency transfers, it directly influences the outcomes of the patient, especially for time-sensitive conditions such as trauma, cardiac emergencies, and stroke. Analysing ambulance transfer efficiency is essential for improving the quality of emergency care, ensuring an adequate number of available ambulances, and strengthening the system. In addition to efficiency, ambulance transfer volume is a key factor for better ambulance management and resource allocation [@chen2015demand].
...
#| echo: false
#| fig-cap: "Histogram of an ambulance transfer distance for emergency and scheduled cases."
...
From @fig-distance-dis, there is zero distance transfer in the data, which suggests either a data entry error or that the aged care facilities and hospital are next to each other (co-located RACF). In the latter case, this might suggest that it may not need the ambulance to transfer the patient. After filtering the distance data to zero and manually going through them, it can be seen that the location of the aged care facility and hospital is, in fact, the same location.

In an emergency case, it is important that the transfer is done quickly (@sec-background). It means that most of the time, the RACF that is co-located with the hospital should not have a patient transferred to another hospital. One thing that will need to be considered is that the co-located hospital might not be able to handle the emergency case. In this case, the patient should be transferred to the closest emergency-capable hospital. This information can be verified through the hospital data, which has a column indicating whether the hospital has an emergency department or not.

Based on this information, the efficiency can be improved through fewer transfers to the other hospital that is further away. especially the co-located RACF. To check this, the distance is calculated between the RACFs and hospitals for all the possible combinations to see which RACF is next to which hospital.
...
patient_dist |>
  ...
  knitr::kable(caption = "Co-located RACF")
...
patient_dist |>
  ...
  st_drop_geometry() |>
  filter(n > quantile(n, probs = 0.95)) |>
  knitr::kable(caption = "Non co-located RACF")
```
...
#| echo: false
#| fig-cap: "An ambulance transfer distance distribution for emergency and scheduled cases in the co-located RACF."
patient_dist |>
  ggplot(aes(x = distance + 1)) +
  geom_histogram(bins = 45) +
...
The expectation of the transfer distance for the co-located RACF would be, on average, less than that of the other case. However, from @fig-distance-dis-close, it can be seen that, on average, the transfer distance for the co-located RACF is higher. It actually raises more questions about why this is the case.
...
#| echo: false
#| warning: false
#| fig-cap: "The co-located RACF transfer network."
simple_graph |>
  activate(edges) |>
...
#| warning: false
#| echo: false
#| fig-cap: "The location of the RACF with the co-located RACF highlighted."
simple_graph |>
...
#| fig-cap: "The co-located RACF transfer network with Melbourne hospital is grouped."
simple_group_graph |>
  activate(edges) |>
  mutate(from_name = .N()$name[from]) |>
  filter(from_name %in% toupper(close)) |>
...
#| echo: false
#| fig-cap: "The co-located RACF transfer network with Melbourne hospital is grouped on the non-spatial layout."
simple_group_graph |>
  activate(edges) |>
  mutate(from_name = .N()$name[from]) |>
  filter(from_name %in% toupper(close)) |>
...
#| echo: false
#| fig-cap: "Histogram of an ambulance transfer distance for emergency and scheduled cases, removing zero distances."
patient_dist |>
  filter(distance != 0) |>
  ggplot(aes(x = distance + 1)) +
  geom_histogram(bins = 45) +
...
### Data cleaning and subsetting
  ...
  - Apply the above software on [Caribou dataset](https://github.com/rfordatascience/tidytuesday/blob/main/data/2020/2020-06-23/readme.md), with the main focus on the temporal data gap that affects total distance travelled calculations.

  - Considering imputation strategies for temporal network data to address missingness.
...
### Exploratory data analysis
  ...
  - Preliminary analysis of the ambulance transfer efficiency
...
There is a wide range of statistical network models, and the choice of model depends on the specific application. In infectious disease research, understanding the underlying contact network is therefore particularly important [@silk2017application].
...
There has also been research using a spatio-temporal point process model to study ambulance demand rather than modelling the network structure. However, such work has primarily focused on ambulance deployment planning rather than transmission or transfer networks [@zhou2015spatio].
...
prj1_data <- tribble(
  ...
  "Developing network EDA infrastructure", 13, 24,
...
prj_data <- bind_rows(prj1_data, prj2_data, thesis_data)
