...
In an emergency case, it is important that thet transfer is done quickly (@sec-background). It means that most of the time, the RACF that is co-located with the hospital should not have a patient transferred to another hospital. One thing that will need to be considered is that the co-located hospital might not be able to handle the emergency case. In this case, the patient should be transferred to another closest emergency-capable hospital. This information can be verified through the hospital data, which has a column indicating whether the hospital has an emergency department or not.
...
The expectation of the transfer distance for the co-located RACF would be, on average, less than that of the other case. However, from @fig-distance-dis-close, it can be seen that, on average, the transfer distance for the co-located RACF is higher. It actually raises more questions to why this is the case.
...
#| fig-cap: "The co-located RACF transfer network with Melbourne hospital grouped."
simple_group_graph |>
  activate(edges) |>
  mutate(from_name = .N()$name[from]) |>
  filter(from_name %in% toupper(close)) |>
...
#| echo: false
#| fig-cap: "The co-located RACF transfer network with Melbourne hospital grouped on the non-spatial layout."
simple_group_graph |>
  activate(edges) |>
  mutate(from_name = .N()$name[from]) |>
  filter(from_name %in% toupper(close)) |>
...
### Data cleaning and subsetting
  ...
  - Apply these software on [Caribou dataset](https://github.com/rfordatascience/tidytuesday/blob/main/data/2020/2020-06-23/readme.md), with the main focus on temporal data gap that affect total distance travelled calculations.
