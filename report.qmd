...
#| fig-cap: "The sampled co-located RACF transfer network, where the Melbourne hospital is grouped."
simple_group_graph |>
  activate(edges) |>
  mutate(from_name = .N()$name[from]) |>
...
#| echo: false
#| fig-cap: "The sample co-located RACF transfer network, where the Melbourne hospital is grouped on the non-spatial layout."
simple_group_graph |>
  activate(edges) |>
  mutate(from_name = .N()$name[from]) |>
  filter(from_name %in% sample_racf) |>
...
From @fig-group-close-net-spatial and @fig-group-close-net-free, it can be observed that many patients are transferred from regional areas to Melbourne. It explains why the median travel distance is longer, as trips from regional RACFs to Melbourne hospitals cover a greater distance than transfers within Melbourne itself. Following this, transfer reasons, such as dispatch type diagnosis, will be explored. To make comparison and interpretation easier, the transfer distance will be grouped into the following;

- Zero distance (co-located facilities)
...
From the tables, the dispatch and diagnosis reasons seem to point to the same thing, which is that for the shorter distance, the reason seems to be about the injuries that require quick transfer, while the medium to long-distance suggests mental health issues. It is expected that the emergency transfer that requires quick attention should be done quickly.

### Next step

  - Use actual road network distances to get more accurate travel measurements.

  - Conduct a more in-depth analysis of differences between emergency and scheduled transfers.

  - Analyse the potential overuse of ambulance callouts.

  - Examine the temporal patterns, such as peak demand for transfers.

  - Assess the impact of COVID-19 on transfer patterns

  - Consider the implications for policymakers, such as the effects of a hospital shutdown.
...
### Project 1A) Data cleaning and subsetting
  ...
  - Explored multiple network analysis software, including `tidygraph`, `igraph`, and `network` for comparison (@sec-storage).
  ...
  - Considering imputation strategies for temporal network data to address missingness (@sec-storage).
...
### Project 1A) Data visualisation
  ...
  - Explored `ggiraph` for interactive network visualisations (@sec-visualisation).

### Project 1B) Exploratory data analysis

  - Preliminary analysis of the ambulance transfer efficiency (@sec-transferdist)
