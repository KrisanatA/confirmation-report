...
### Next steps
  ...
  - The work will result in an R package that provides a data object that better encapsulates the spatio-temporal network structure, along with the necessary functionality. It will be designed to work with a tidyverse suite. The R package output is expected to turn into a publishable paper.
...
### Next steps
  ...
  - The work of Part B will help inform the development of the new data package, as well as applying the new package to the analysis. The analysis of the COVID-19 impact on the ambulance transfers will be turned into a publishable paper.
...
### Project 1A) Data cleaning and subsetting
  ...
  - I have explored multiple network analysis software, including `tidygraph`, `igraph` and `network` for comparison (@sec-storage). This helps identify the limitations of the current tools and what is needed in the new data object along with its functionality.
...
- Paper 1: Understanding the spatio-temporal network dynamics on aged care ambulance transfers in Victoria.

- Paper 2: A new tidy data structure for spatio-temporal multivariate network exploration and modelling.

- Paper 3: Modelling infectious disease dynamics using a generalised ambulance model.
...
## Other activities
  ...
  - I run a tutorial/workshop with Jayani and Janith on reproducible research in WOMBAT2025 and as part of the NUMBAT workshops. The tutorial introduces Quarto for creating reproducible reporting, academic documents and presentations.
...
This confirmation is in the following [repositories](checkfirst). I am commit to using GitHub version control and quarto (with R) for reproducible research. 
