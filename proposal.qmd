...
Another important aspect of subsetting is understanding how sampling methods perform on network data. Real-world datasets are generally not evenly distributed across dimensions such as time, space, or variable groups. Some strata may contain a larger number of observations than others. Analysing such data can directly impact the interpretation, as the larger group of strata can dominate the pattern seen. Sampling provides a way to address this issue. Stratified sampling in particular helps with these issues by dividing the data into subgroups and sampling within each group to ensure that all groups are represented within the sample. 

In the network context, they are generally categorised into three main classes:

  - Node-based sampling selects a subset of nodes from the network and retains edges that connect to the sampled node. It may fail to capture important global structural properties such as connectivity and clustering.
  
  - Edge-based sampling samples a subset of edges directly and includes the nodes incident to those edges. It is better for preserving structural patterns, but may introduce bias into the sample node set and distort centrality measures.

  - Exploration-based sampling constructs samples by traversing the network, for example, random walks or snowball sampling. It is good at preserving local connectivity and community structure, but can be sensitive to the initial seeds, which lead to an overrepresentation of densely connected regions.
